...
A tenant submits an SBOM (pinned to an image digest) via an authenticated API. A Cloud Run Job rescans due SBOMs (on a frequent schedule, each SBOM at most ~twice a day — see the Scan Job below) with Grype and Trivy, normalizes results into a scanner-agnostic schema, and records every change as an append-only event. The product is the **delta over time**, not point-in-time scan results.
...
2. **Scan Job** (Cloud Run Job `devradar-saas-scan`, pure CPU) — for each *due* SBOM, runs Grype + Trivy on the SBOM file, normalizes, writes current state + change events. Triggered by Cloud Scheduler on a frequent cadence (default every 15 min, `var.scan_schedule`); work selection filters to SBOMs never scanned or last scanned older than `DEVRADAR_SCAN_MAX_AGE` (default 12h) via `ListScannableSBOMs`. So a freshly-submitted SBOM is picked up on the next tick (low latency) while any given SBOM is rescanned at most ~twice a day (bounded load) — cron frequency controls latency, the staleness window controls load, independently. The UI shows a scan heartbeat ("Last scan N min ago · scans run every 15 min") so a pending scan never looks broken.
