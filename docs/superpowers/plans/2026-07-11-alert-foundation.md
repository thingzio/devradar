...
func TestAlertMigration_DefaultsAndConstraints(t *testing.T) {
	...
	if p.Enabled || p.MinSeverity != data.SeverityMedium || !p.AlertKEV ||
		!p.AlertFixAvailable || !p.IncludeImage || !p.IncludeDB || len(p.Labels) != 0 {
