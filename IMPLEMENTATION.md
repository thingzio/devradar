...
> **The authoritative API contract is the OpenAPI spec** — served at `/openapi.yaml` and rendered human-readably at `/api` (both public). Format auto-detection is used unconditionally (there is no `format_hint` field). This table is a design-time summary; the spec is generated from the live handler shapes and is the source of truth for every endpoint, parameter, and schema.
...
**VEX suppression** is applied consistently across *every* finding read and count (SBOM findings, image/fleet rollups, package rollup) so the numbers a `not_affected` statement changes are the same everywhere. On per-SBOM findings, suppressed rows are hidden by default and revealed with `?suppressed=true`. On the **fleet CVE list**, VEX'd CVEs are instead **shown and annotated** (status + justification + impact statement, visually de-emphasized) so the triage context is visible; that list is filterable by VEX status, justification, KEV, and fixability. A CVE is attributed a VEX status only when a statement applies to *every* occurrence (a partially-VEX'd CVE still reads as open). VEX can be uploaded via `POST /v1/vex` (API) or the CVEs tab (browser upload). A statement scopes either to a **specific digest** (`pkg:oci/name@sha256:...` — version-precise) or, when digest-less (`pkg:oci/name`), to a **repository** across all its versions — the common real-world pattern for `vulnerable_code_not_in_execute_path` justifications, matched on the repository's last path segment so registry-prefix differences don't matter. OpenVEX `subcomponents` (per-package) are stored but not yet used for matching.
...
**Pagination.** List endpoints use **keyset (seek) pagination**, not `OFFSET`: the cursor is an opaque base64 token carrying the last row's ordering key plus a unique tiebreaker. Each response includes `next_cursor` only when a further page exists; pass it back as `?cursor=`. Default page size 100, max 1000. Keyset avoids `OFFSET`'s O(n) scan and its skip/duplicate anomalies under concurrent writes — important because the event log (CUJ-3) is the unbounded, high-volume table.

**Server-side sorting.** Sortable list endpoints (`/v1/sboms/{id}/findings` by severity/cvss/epss/package/cve/scanner/fixable; fleet CVEs by risk/severity/images/findings/epss/cve; per-image SBOMs by generated/version/packages) take `?sort=<col>&dir=asc|desc` from a per-endpoint whitelist. Sorting composes with keyset pagination: the ORDER BY is always `(<sort-expr> DIR, <unique-tiebreak> DIR)` and the cursor carries the sort column's value (as text) so the seek predicate stays a single row-comparison. Sort expressions are `COALESCE`'d non-null (a NULL keyset breaks the seek). The UI renders sortable column headers that toggle direction and persist the sort across pages.
...
- `/findings` and `/events` **omit rows** below the threshold (`severity = ANY(allowed)`).
- `/v1/images` **keeps every tracked image** (the list is an inventory — images must not vanish when a threshold rises) but **trims each image's `counts` breakdown** to levels at or above the threshold; sub-threshold buckets are dropped from the JSON entirely (`omitempty`, not rendered as `0`). `relevant` sums the visible buckets and `total` stays the overall finding count (both always present), so a caller can still tell lower-severity findings exist.

**`unknown` is always included**, at any threshold — an unrated CVE could be anything, so it is never hidden. The ordering lives in one place (`data.SeverityRank` / `AllowedSeverities` / `MeetsThreshold`, plus `applyThreshold` for the image breakdown) so the row filter and the count trimming can't drift. `unknown` is not a valid *threshold* value (it's a floor of "everything ranked", not a rank).
